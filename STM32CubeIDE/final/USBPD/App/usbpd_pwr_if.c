...
/* USER CODE BEGIN USBPD_PWR_IF_ReadVA */
  uint32_t v; int32_t i;
  if ((BSP_USBPD_PWR_VBUSGetVoltage(PortNum, &v) == BSP_ERROR_NONE) && (BSP_USBPD_PWR_VBUSGetCurrent(PortNum, &i) == BSP_ERROR_NONE))
  {
    if (pVoltage) *pVoltage = (uint16_t)((v > 0xFFFFu) ? 0xFFFFu : v);
    if (pCurrent) *pCurrent = (uint16_t)((i < 0) ? 0 : ((i > 0xFFFF) ? 0xFFFF : i));
    return USBPD_OK;
  }
  return USBPD_ERROR;
...
/* USER CODE BEGIN USBPD_PWR_IF_SearchRequestedPDO */
  if ((0u == RdoPosition) || (NULL == Pdo))
  {
    return USBPD_FAIL;
  }
  // RdoPosition is 1-based index in source PDO list
  uint32_t index = RdoPosition - 1u;
  if (index >= USBPD_NbPDO[1])
  {
    return USBPD_FAIL;
  }
  *Pdo = PORT0_PDO_ListSRC[index];
  return USBPD_OK;
